...
read_ini() {
    ...
    # Set defaults
    local BOOLEANS=1
    local VARNAME_PREFIX=$INI_VARNAME_PREFIX
    local CLEAN_ENV=0
    ...
    if [ -z "$INI_FILE" ] && [ "${CLEAN_ENV}" = 0 ] ;then
        echo -e "Usage: ${FUNCNAME[0]} [-c] [-b 0| -b 1]] [-p PREFIX] FILE"\
    ...
    eval ${INI_ALL_VARNAME}=
    eval ${INI_ALL_SECTION}=
    eval ${INI_NUMSECTIONS_VARNAME}=
...
get_ini_var() {
    local USAGE="Usage: ${FUNCNAME[0]} SECTION VARNAME"
    if (($# < 2)); then
        echo "$USAGE" >&2
        return 1
    fi
    local section="$1"
    local varname="$2"
    local varnamefull=${INI_VARNAME_PREFIX}__${section}__${varname}
    echo ${!varnamefull}
}
